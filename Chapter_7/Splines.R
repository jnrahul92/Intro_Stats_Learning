...
library(splines)
fit = lm(wage~bs(age,knots = c(25,30,60)),data = Wage)
pred = predict(fit, newdata = list(age = age.grid),se=T)
se.bands = cbind(pred$fit+2*pred$se.fit,pred$fit-2*pred$se.fit)
plot(age,wage,col="gray")
lines(age.grid, pred$fit,lwd=2)
matlines(age.grid,se.bands,lwd = 1, lty = 3,col="black")
...
legend("topright",legend = c("16 DF","6.8 DF"),col = c("red","blue"), lty = 1, lwd = 3, cex = 0.8)
...
legend("topright",legend = c("Span = 0.2","Span 0.5"),col = c("red","blue"), lty = 1, lwd = 3, cex = 0.8)
