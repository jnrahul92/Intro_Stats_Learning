...
knn.out = kmeans(x,2,nstart = 20)
par(mfrow=c(1,1))
plot(x, col = (knn.out$cluster+1), main = "K-Means Clustering Results with k = 2",
     xlab = "", ylab = "",pch = 20, cex = 2)

set.seed(4)
knn.out = kmeans(x,3,nstart = 20)
knn.out

set.seed(3)
knn.out = kmeans(x,3,nstart = 1)
knn.out$tot.withinss
knn.out = kmeans(x,3,nstart = 20)
knn.out$tot.withinss
...
km.cluters = km.out$cluster
table(km.cluters, hc.clusters)
